...
$(function () {
  ...
  (function () {
    var extractYoutubeId = function (url) {
      ...
      if (/^https?:\/\/.*youtube.*/i.test(url)) {
        ...
        /(?:^|")(?:https?:\/\/|)(?:www\.)?youtube\.com\/(?:(?:embed\/)|(?:watch\?v=))(.+?)(?:"|$)/i.test(
          url,
        )
      ) {
        id = RegExp.$1;
  ...
  var openPopupPageWidth =
    "openPopupPageWidth" in window && window.openPopupPageWidth;
  var openPopupPageHeight =
    "openPopupPageHeight" in window && window.openPopupPageHeight;
  ...
  var SpMenu = function (elem, parent) {
    ...
    this.isWrapper = function () {
      return this._wrapper ? true : false;
    };
    /**
    ...
    this.getWidth = function () {
      return menuElem.outerWidth(true);
    };
      ...
      if (currentPageItems.length < 2) {
        ...
        if (!searchFunc(anchorsAbove))
          if (!searchFunc(anchorsBelow))
            // Fallback if there are no menu items pointing to an anchor above current window scroll position (including menu item without anchor).
  ...
  (function () {
    ...
    var updateMainBlockHeight = function () {
      ...
      if (!isPopupMode) {
        ...
        minMainBlockHeight =
          minMainBlockHeight - headerHeight - footer.outerHeight(true);
      }
  ...
  (function () {
    ...
    window.initLink = function (link) {
      ...
      if (func) $link.off("click", func); // make sure that event is no longer listened since store gallery reuses same link element
